$(function () {
    ...
    $('.data-table thead th').each(function (index) {
        ...
        if(index >= 0 && index < thLen - 1){
            ...
            if(col_type == 'text'){
                $('#searchable-row').append('<th class="text-center"><input type="text" class="text-center filter-table"/></th>');
            }
            if(col_type == 'no_filter'){
                $('#searchable-row').append('<th class="text-center"></th>');
            }
            if(col_type == 'name'){
                $('#searchable-row').append('<th class="text-center"><input type="text" class="text-center filter-table" style="width:130px"/></th>');
            }
            if(col_type == 'date'){
                $('#searchable-row').append(`<th class="text-center"><input type="text" class="text-center filter-table date-range" style="width:100px"/></th>`);
            }
            if(col_type == 'select'){
            ...
            if(index == 0){
                ...
                $('#searchable-row').append('<th class="align-middle text-center"><div style="width: 80px"></div></th>');
            }
        }
    });
    ...
    $('body').on('change', 'input[type="checkbox"]', function() {
        ...
        $.ajax({
            type: "GET",
            url: url + '/merchant/offers/change-status/' + offerID,
            data: {},
            success: function(data)
            {
                if(data.results == 'activate'){
